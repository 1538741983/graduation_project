...
namespace OJCMS.User
{
    public partial class UserManager : PageBase
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                //BtnSelectDel.OnClientClick += Grid1.GetNoSelectionAlertInParentReference("请至少选择一项");
                //BtnSelectDel.ConfirmText = "确定选中记录？";

                btnNew.OnClientClick = WindowDialog.GetShowReference("~/User/UserNew.aspx", "新增用户");// Window1.GetShowReference("UserNew.aspx", "新增用户");
        ...
        private UserPageList LoadData()
        {
            int pageIndex = Grid1.PageIndex;
            int pageSize = Grid1.PageSize;
            string searchText = ttbSearchMessage.Text.Trim();
            IOJService ojService = new OJServiceClient();
            return ojService.GetUserPageList(pageSize, pageIndex, searchText);
        }

        private void BindGrid(UserPageList userPageList)
        {
            //获取总条目
            Grid1.RecordCount = userPageList.RecordCount;

            UserDO[] userList = userPageList.DataList;
            //获取分页数据
        ...
        protected void Grid1_RowCommand(object sender, GridCommandEventArgs e)
        {
            ...
            int userId = int.Parse(values[0].ToString());
            string guname = values[1].ToString();
